...
All NixOS hosts decrypt the same `secrets/me.yaml` with the same age identity (now at `/var/lib/sops-nix/keys.txt`, see ADR 0008; originally `~/.config/sops/age/keys.txt`), sole recipient in `.sops.yaml`, copied manually to each host. Per-host identities were considered and rejected.
